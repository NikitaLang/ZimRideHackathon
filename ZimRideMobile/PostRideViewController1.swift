...
enum RoleType:uint {
    case driver = 1, passenger = 2
}

class PostRideViewController1: UIViewController {
    
    var viewWidth:Double!
    var navBar:UIView!
    var rolePanel:UIView!
    ...
    func initRolePanel(){
       ...
       self.rolePanel = UIView.init(frame: CGRect.init(x: 10, y: 140, width: self.viewWidth, height: panelHeight))
        self.view.addSubview(self.rolePanel)
        ...
        leftButton.tag = Int(RoleType.passenger.rawValue)
        leftButton.backgroundColor = .white
        leftButton.setTitle("A Passenger", for: .normal)
        leftButton.setTitleColor(.lightGray, for: .normal)
        leftButton.setTitleColor(.black, for: .selected)
        leftButton.addTarget(self, action: #selector(selectRole(_:)), for: .touchUpInside)
        leftButton.layer.cornerRadius = 4
        self.rolePanel.addSubview(leftButton)
        self.rolePanel.isUserInteractionEnabled = true
        
        let rightButton = UIButton.init(frame: CGRect.init(x: viewWidth - 35 - buttonWidth, y: 10, width: buttonWidth, height: buttonHeight))
        rightButton.tag = Int(RoleType.driver.rawValue)
        rightButton.backgroundColor = .white
        rightButton.setTitle("Willing to Drive", for: .normal)
        rightButton.setTitleColor(.lightGray, for: .normal)
        rightButton.setTitleColor(.black, for: .selected)
        rightButton.addTarget(self, action: #selector(selectRole(_:)), for: .touchUpInside)
        rightButton.layer.cornerRadius = 4
        self.rolePanel.addSubview(rightButton)
    }
    
    func intiPathPanel(){
        let pathHeight = 150.0
        
        self.pathPanel = UIView.init(frame: CGRect.init(x: 0, y: 220, width: self.viewWidth, height: pathHeight))
        ...
        let fromTextField = UITextField.init(frame: CGRect.init(x: 130, y: 0, width: self.viewWidth - 160, height: 52))
        ...
        let toTextField = UITextField.init(frame: CGRect.init(x: 130, y: 0, width: self.viewWidth - 160, height: 52))
    ...
    func initMapView(){
        self.mainMapView = MKMapView(frame: CGRect.init(x: 20, y: 370, width: self.viewWidth - 40, height: 200))
        self.view.addSubview(self.mainMapView)
        
        self.mainMapView.layer.cornerRadius = 4
    ...
    @objc func selectRole(_ sender: UIButton){
      let array = self.rolePanel.subviews
        for view in array{
            if view is UIButton{
        ...
        self.roleType = RoleType(rawValue: uint(sender.tag))
        }
    @objc func nextPage(){
...
extension PostRideViewController1:MKMapViewDelegate{
    
}

extension PostRideViewController1:UITextFieldDelegate{
    func textFieldDidEndEditing(_ textField: UITextField) {
        self.addressTableView.isHidden = true
    }
}

extension PostRideViewController1:UITableViewDelegate, UITableViewDataSource{
